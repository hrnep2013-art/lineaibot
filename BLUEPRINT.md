...
| AI Model | `gemini-3.1-flash-lite` (hardcode ไว้ใน `lib/gemini.ts`) — เปลี่ยนจาก `gemini-3.5-flash` เมื่อ 23 ก.ย. 2569 เพื่อหนีโควตา Free Tier 20 ครั้ง/วันของรุ่นเดิม ⚠️ Google ประกาศเลิกใช้รุ่นนี้แล้ว กำหนด 7 พ.ค. 2570 แนะนำย้ายไป `gemini-3.5-flash-lite` ก่อนถึงวันนั้น |
...
4. **ต่อ event หนึ่งอัน (`handleEvent`)**:
   ...
   - **try block:**
     ...
     c. **เรียก Gemini แบบ 2 รอบ (two-pass)**:
        ...
        - **รอบที่ 2** — เปิดเฉพาะตอนรอบแรก "ตอบสำเร็จแต่หาไม่เจอ" เท่านั้น (ข้อความว่าง/`MAX_TOKENS`/ตรงกับ `DEFAULT_REPLY` เป๊ะๆ) **ถ้ารอบแรก error/timeout ไปเลยจะไม่ลองรอบ 2 ต่อ** (ตั้งใจแบบนี้ 23 ก.ย. 2569 — กันใช้โควตา 2 เท่าต่อคำถามตอนระบบมีปัญหาอยู่แล้ว โดยเฉพาะช่วงที่ยังใช้ Free Tier โควตาจำกัด) และมี `GEMINI_FILE_SEARCH_STORE` ตั้งไว้ → เรียกซ้ำพร้อมเปิด `fileSearch` tool timeout 16s
...
10. **เคยเจอจริง (23 ก.ย. 2569): thinkingLevel MEDIUM คิดหนักจนชนเพดานได้เองแม้ใน "รอบที่ 1" ที่ไม่เปิด File Search เลย** (เจอ `thoughtsTokenCount: 1965/2048` พร้อม timeout ทั้งที่ `useFileSearch: false`) ยืนยันว่าปัญหาการคิดเยอะเกินไม่ได้ผูกกับ File Search อย่างเดียว เป็นคุณสมบัติที่ค่อนข้างไม่แน่นอนของ thinkingLevel MEDIUM เอง เคยลองแก้ด้วยการให้ลองรอบที่ 2 ต่อแม้รอบแรกจะ timeout/error ไปเลย แต่ **revert กลับไปแล้วในวันเดียวกัน** ตามคำขอ เพราะตอนระบบมีปัญหาโควตา/timeout อยู่แล้ว การลองซ้ำ 2 รอบจะยิ่งใช้โควตา 2 เท่าต่อคำถาม ซ้ำเติมปัญหา — ตอนนี้ถ้ารอบแรก error/timeout ไปเลยจะไม่ลองรอบ 2 ต่อ (ดูรอบที่ 2 ในหัวข้อ 2) ถ้าพบว่าคำถามบางแบบ (เช่น พิมพ์ยาว/อ้อมค้อม/หลายประเด็นในคำถามเดียว) ทำให้เกิดซ้ำบ่อย ควรพิจารณาปรับพรอมต์ให้กระชับ/ชัดเจนขึ้น หรือทดลองปรับ thinkingLevel ใหม่
11. **เปลี่ยนโมเดลจาก `gemini-3.5-flash` เป็น `gemini-3.1-flash-lite` เมื่อ 23 ก.ย. 2569** เพื่อหนีปัญหาโควตา Free Tier ของรุ่นเดิม (20 ครั้ง/วัน) — รุ่นใหม่รองรับ thinking และ File Search ครบเหมือนเดิม แต่**ยังไม่ยืนยันตัวเลขโควตา Free Tier ที่แน่ชัด** และเป็นรุ่น "lite" ที่ Google ออกแบบมาสำหรับงานง่าย/ปริมาณเยอะ ไม่ใช่งานที่ต้องคิดวิเคราะห์ซับซ้อนแบบบอทนี้ **ควรสังเกตคุณภาพคำตอบ + อัตรา fallback ใน log Sheet หลังเปลี่ยนอย่างใกล้ชิด** ว่าแย่ลงจากตอนใช้ `gemini-3.5-flash` ไหม นอกจากนี้ Google ประกาศเลิกใช้ `gemini-3.1-flash-lite` แล้ว กำหนดวันที่ 7 พ.ค. 2570 แนะนำย้ายไป `gemini-3.5-flash-lite` ก่อนถึงวันนั้น — **ทางแก้ถาวรที่แนะนำจริงๆ คือกลับไปใช้ `gemini-3.5-flash` เดิมพร้อมอัปเกรดบัญชีเป็น Paid Tier** (ดูวิธีที่คุยกันไว้ก่อนหน้า) ไม่ใช่การเปลี่ยนโมเดลหนีไปเรื่อยๆ
12. **ชื่อโมเดล `gemini-3.1-flash-lite` hardcode ไว้จุดเดียว** ใน `lib/gemini.ts` ไม่มี fallback ถ้า Google เปลี่ยน/เลิกซัพพอร์ตชื่อนี้ (ดูข้อ 11 — รุ่นนี้มีกำหนดเลิกใช้แล้ว 7 พ.ค. 2570)
13. **ไม่มี automated test เลยในโปรเจกต์นี้** ตรวจสอบทุกครั้งด้วย `tsc --noEmit` + `next build` + ทดสอบจริงบน LINE มือ
14. **สมมติฐานเรื่อง Vercel Hobby plan ไม่ตรงกับที่เจอจริง** — คอมเมนต์เก่าในโค้ดเคยเขียนว่า Hobby จำกัด 10s แต่ log จริงแสดงว่าใช้ได้ถึง 30s (อาจเพราะ Fluid Compute) ยังไม่ได้ไปยืนยันสาเหตุแน่ชัด ถ้า deploy เริ่ม timeout ผิดปกติให้กลับมาเช็คจุดนี้
15. **`getThaiDateString()` ใน `lib/prompt.ts` ใช้ offset UTC+7 ตายตัว** ไม่ได้ใช้ timezone library จริงจัง — ใช้ได้เพราะไทยไม่มี DST แต่ถ้าจะเอาโค้ดนี้ไปใช้ที่อื่นต้องระวัง
